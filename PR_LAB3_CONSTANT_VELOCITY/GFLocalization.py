...
class GFLocalization(Localization,GaussianFilter):
    ...
    def Localize(self, xk_1, Pk_1):
        ...
        uk, Qk = self.GetInput()
        # print(f"The shape of uk and Qk are {uk.shape} and {Qk.shape} respectively")
        xk_bar, Pk_bar = self.Prediction(uk,Qk,xk_1,Pk_1)
        # print(f"The shape of xk_bar and Pk_bar is {xk_bar.shape} and {Pk_bar.shape} respectively")
        zk, Rk, Hk, Vk, received_encoder_readings, recieved_heading_data = self.GetMeasurements()
        # print("This is Pk_bar just before update call",Pk_bar)
        if zk is None:
            xk, Pk = xk_bar, Pk_bar
            # had to add the following line because then plotting was not working
            self.xk, self.Pk = xk_bar, Pk_bar
        else:
            xk, Pk = self.Update(zk,Rk,xk_bar,Pk_bar,Hk,Vk, received_encoder_readings, recieved_heading_data)
        # print(f"The current time in the simulation is {self.k*self.robot.dt}")
        # print(f"The shape of xk and Pk is {xk.shape} and {Pk.shape} respectively")
    ...
    def PlotState(self):
        ...
        for s in range(self.log_x.shape[0]):  # for each estimated state
            ...
            if self.index[s].simulation is not None and s<self.log_xs.shape[0]:  # there is a corresponding simulated state
                ...
                if s == 2: # Or you can check: if self.index[s].state == 'yaw':
                    # We need to apply wrap_angle to every element in the error array
                    # The easiest way is to "vectorize" the function
                    wrap_angle_v = np.vectorize(wrap_angle) 
                    e = wrap_angle_v(e)

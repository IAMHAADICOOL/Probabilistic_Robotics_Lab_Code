...
class EKF_3DOFDifferentialDriveInputDisplacement(GFLocalization, DR_3DOFDifferentialDrive, EKF):
    ...
    def f(self, xk_1, uk):
        ...
        # print(f"This is the shape of eta_k_1 {eta_k_1.shape} and this is the shape of nu_k {nu_k.shape}")
        # print(f"This is the type of eta_k_1 {type(eta_k_1)} and this is the shape of nu_k {type(nu_k)}")
        xk_bar = Pose3D.oplus(xk_1, Pose3D(uk))
        # construct the new state vector
        # xk_bar = np.vstack((eta_k.reshape(3,1), nu_k.reshape(3,1)))  # new robot state
        # self.k += 1
...
# there is a problem in the computation of the Jacobians
    ...
    def h(self, xk):  #:hm(self, xk):
        # TODO: To be completed by the student
        # not sure if following implementations are correct
        # not able to understand why this is working?
        h = xk[2,0]
    ...
    def GetInput(self):
        ...
        """
        # TODO: To be completed by the student
        # I don't think we have implement this method because it is already implemented in the DR_3DOFDifferentialDrive class
...
#                       [1/L, 0, 1/L]]=D, we have
        ...
        # now this displacement is wrt the robot frame, we need actual discplacement
        # wrt world frame (M)
        # psi = self.xk_1[2,0]
        # [delta_x_M, delta_y_M, delta_theta_M]^T = [[Cos(psi), -Sin(psi), 0],
        #                                           [Sin(psi), Cos(psi), 0],
        #                                           [0, 0, 1]]@[delta_x, delta_y, delta_theta]^T
        # Assumming F = [[Cos(psi), -Sin(psi), 0],
        #                [Sin(psi), Cos(psi), 0],
        #                [0, 0, 1]]
        # now the covariance of error in delta_x, delta_y, delta_theta is Q_displacement, therefore,
        # using the magic table, the covariance of error in displacement wrt world frame is going to be
        # Q_final = F*Q_displacement_F^T
        psi = self.xk_1[2,0]
        F = np.asarray([[cos(psi), -sin(psi), 0],
                        [sin(psi), cos(psi), 0],
                        [0, 0, 1]])
        # Q_final = F @ Q_displacement @ F.T

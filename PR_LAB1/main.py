...
if __name__ == "__main__":
    ...
    xsk_1 = xs0 = np.zeros((6, 1))  # initial simulated robot pose and previous robot state
    index = [IndexStruct("x", 0, None), IndexStruct("y", 1, None), IndexStruct("yaw", 2, 1)] # index of the state vector used for plotting

    x0=Pose3D(np.zeros((3,1))) # initial robot pose estimate
    dr_robot=DR_3DOFDifferentialDrive(index,kSteps,robot,x0)
    dr_robot.LocalizationLoop(x0, np.array([[0.5,0, 0.03]]).T)

...
def p2c(p):  # polar to cartesian conversion
    ...
    """
    if isinstance(p, np.ndarray) and isinstance(f.PolarFeature, type):
        p = p.reshape(2, 1)  # Ensure p is a column vector
        rho, theta = p[0,0], p[1,0]
    elif (isinstance(p, list) or isinstance(p, tuple)) and isinstance(f.PolarFeature, type):
        rho, theta = float(p[0]), float(p[1])
    x = rho * cos(theta)
    y = rho * sin(theta)
...
def c2p(c):  # cartesian to spherical conversion
    ...
    """
    if isinstance(c, np.ndarray) and isinstance(f.CartesianFeature, type):
        c = c.reshape(2, 1)  # Ensure c is a column vector
        x, y = c[0,0], c[1,0]
    elif (isinstance(c, list) or isinstance(c, tuple)) and isinstance(f.CartesianFeature, type):
        x, y = float(c[0]), float(c[1])
    rho = np.sqrt(x**2 + y**2)
    theta = atan2(y, x)
    return f.PolarFeature(np.array([rho, theta]).reshape(2,1))
...
def J_c2p(c):  # cartesian to spherical conversion
    ...
    rho_squared = x**2 + y**2
    J = np.array([
        [x / np.sqrt(rho_squared), y / np.sqrt(rho_squared)],
        [-y / rho_squared, x / rho_squared]
    ])
    return J
...
def s2c(s):  # spherical to cartesian conversion
    ...
    if isinstance(s, np.ndarray):
        s = s.reshape(3, 1)  # Ensure s is a column vector
        rho, theta, phi = s[0, 0], s[1, 0], s[2, 0]
    elif isinstance(s, list) or isinstance(s, tuple):
        rho, theta, phi = float(s[0]), float(s[1]), float(s[2])
...
def J_s2c( s):  # spherical to cartesian conversion
    ...
    if isinstance(s, np.ndarray):
        ...
        rho, theta, phi = float(s[0]), float(s[1]), float(s[2])
    J = np.array([
        [np.sin(theta) * np.cos(phi), rho * np.cos(theta) * np.cos(phi), -rho * np.sin(theta) * np.sin(phi)],
        [np.sin(theta) * np.sin(phi), rho * np.cos(theta) * np.sin(phi), rho * np.sin(theta) * np.cos(phi)],
        [np.cos(theta), -rho * np.sin(theta), 0]
    ])
    return J    
...
def J_c2s(c):  # cartesian to spherical conversion
    ...
    J = np.array([
        ...
        [-x*z/((x**2 + y**2)*np.sqrt(x**2 + y**2)), -y*z/((x**2 + y**2)*np.sqrt(x**2 + y**2)), np.sqrt(x**2 + y**2)/(x**2 + y**2)]
    ])

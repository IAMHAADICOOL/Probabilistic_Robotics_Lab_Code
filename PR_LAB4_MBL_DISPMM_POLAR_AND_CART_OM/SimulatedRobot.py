    ...
    **All Robot simulation classes must derive from this class** .
    ...
    def __init__(self, xs0, map=[], * args):
        ...
        * **visualizationInterval** : time-steps interval between two consecutive frames of the animation
        ...
        self.vehicleIcon = VehicleIcon(str(pathlib.Path(__file__).parent.resolve())+'/DifferentialDrive.png', scale=5, rotation=90) # Image of the robot to be used in the animation. By default it uses the image of a Differential Mobile Robot.
        ...
        for i in range(len(map)):
            mf = map[i].ToCartesian() if isinstance(map[i], Feature) else map[i]
            feature_circle.append(patches.Circle((float(mf[0]), float(mf[1])), 0.5, fc='r'))

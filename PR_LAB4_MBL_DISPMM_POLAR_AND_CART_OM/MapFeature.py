...
class MapFeature:
    ...
    * **The storage representation:** which is the representation used to store the feature in the map within the state vector.
    ...
    def GetFeatures(self):
        ...
        # TODO**This is a pure virtual method that must be overriden in child classes**.
    ...
    def s2o(self, v):
        ...
        # TODO: In case it is not, this method must be overriden in the child class.
    ...
    def o2s(self, v):
        ...
        # TODO: In case it is not, this method must be overriden in child classes.
    ...
    def J_s2o(self, v):
        ...
        # TODO: In case it is not, this method must be overriden in the derived class.
    ...
    def hf(self, xk):  # Observation function for al zf observations
        ...
        """
        # TODO: To be implemented by the student
        # self.H defined in nFEKFMBL child class
        # may throw error here due to the way H is defined in nFEKFMBL
        # TODO: compute n_zf, that is, number of features observed
        _hf_list = []
        
        # Iterate over the association hypothesis vector self.H
        # self.H might be None if not initialized, so we should check
        if hasattr(self, 'H') and self.H is not None:
            for Fj in self.H:
                if Fj is not None:
                    _hf_list.append(self.hfj(xk, Fj))
        
        # FIX: Check if list is empty before vstacking
        if len(_hf_list) > 0:
            _hf = np.vstack(_hf_list)
        else:
            # Return an empty column vector (0 rows, 1 column) to allow successful stacking
            # with the compass measurement in the parent class.
            _hf = np.zeros((0, 1))
            
        return _hf

    def Jhfx(self, xk, Fj):  # Jacobian wrt x of the feature observation function for all zf observations
        """
        ...
        # TODO: To be implemented by the student
        jacobians = []
        for i, Fj in enumerate(self.H):  # self.H is matching features hypothesis vector
            if Fj is not None:
                J_feature = self.Jhfjx(xk, Fj)  # Jacobian for single feature observation
                jacobians.append(J_feature)
            else:
                # If no association, append appropriate zero matrix or skip 
                # depending on implementation needs
                pass
        if jacobians:
            J = np.vstack(jacobians)
        else:
            J = np.empty((0, xk.shape[0]))  # empty Jacobian if no features observed
        return J
    ...
    def Jhfv(self, xk):  # Jacobian wrt v of the observation function for a feature
        ...
        # TODO: To be implemented by the student
        # TODO: compute n_zf, that is, number of features observed
        # Defined in FEKFFMBL class, inside the Localise method, 
         # Count number of associated features
        n_associated = sum(1 for h in self.H if h is not None and h != 0)
        identity_size = n_associated * self.zfi_dim
        J = np.eye(identity_size)
        return J
    ...
    def hfj(self, xk_bar, Fj):  # Observation function for zf_i and x_Fj
        ...
        # self.M is defined in the child class FEKFMBL

        NxF = self.M[Fj]
        # TODO: Whether to encapuslate the robot pose within Pose3D class or not
        # print("This is type of xk_bar",type(xk_bar))
        _hfj = self.s2o(NxF.boxplus(xk_bar.ominus()))
        return _hfj
    ...
    def Jhfjx(self, xk, Fj):  # Jacobian wrt x of the observation function for feature observation i
        ...
        #  # Compute the expected observation: hfj = s2o(⊖NxB ⊕ NxFj)
        # BxFj_expected = xk.ominus().boxplus(NxF)

        # # Jacobian chain: J_s2o @ J_1boxplus @ J_ominus
        # J_s2o_val = self.J_s2o(BxFj_expected)
        # J_1boxplus_val = xk.ominus().J_1boxplus(BxFj)
        # J_ominus_val = xk.J_ominus()
        # # TODO: Whehter the following equation is correct or not
        # Jp = self.J_s2o(NxF.boxplus(xk.ominus()))@CartesianFeature.J_1boxplus(NxF, xk.ominus())@xk.J_ominus()
        # Jnp = 0
        # F = np.eye(xk.shape[0])[0:3, :].T
        # J = J_s2o_val @ J_1boxplus_val @ J_ominus_val @ F
        # only returning Jp, not Jnp for this lab

        J=self.J_s2o(NxF.boxplus(xk.ominus()))@NxF.J_1boxplus(xk.ominus())@xk.J_ominus()
        return J
    ...
    def Jgx(self, xk, BxFj):  # Jacobian wrt xk of the inverse sensor model for a single feature observation
        ...
        # TODO: To be implemented by the student
        Jp = self.o2s(BxFj).J_1boxplus(xk)
        # only returning Jp, not Jnp for this lab
        Jnp = 0
        return Jp
...
class Cartesian2DMapFeature(MapFeature):
    ...
    def GetFeatures(self, xk_bar):
        ...
        """
        # TODO: To be implemented by the student
        zf, Rf = self.robot.ReadCartesianFeature(xk_bar)
        # print("This is the type and shape of zf, Rf in GetFeatures:", type(zf), zf.shape if zf is not None else None,
        #       type(Rf), Rf.shape if Rf is not None else None)
        # print(f"This is the type of zf returned from GetFeatures function inside MapFeature.py {type(zf)}")
        return zf, Rf
...
class PolarMapFeature(MapFeature):
    ...
    def GetFeatures(self, xk_bar):
        ...
        """
        # TODO: To be implemented by the student
        zf, Rf = self.robot.ReadPolarFeature(xk_bar)
        # print("This is the type and shape of zf, Rf in GetFeatures:", type(zf), zf.shape if zf is not None else None,
        #       type(Rf), Rf.shape if Rf is not None else None)
        # print(f"This is the type of zf returned from GetFeatures function inside MapFeature.py {type(zf)}")
        return zf, Rf
...
class Cartesian2DStoredPolarObservedMapFeature(MapFeature):
    """
    This class inherits from the :class:`MapFeature` and implements a 2D Cartesian feature stored and observed in Polar model for the MBL problem. The Cartesian coordinates 
    are used for storing the feautres within map and polar coordinates are used for observing the feature. This class overrides the :meth:`GetFeatures` 
    method to read the 2D Cartesian Features from the robot.
    """
    ...
    def s2o(self, v):
        ...
        """
        # TODO: To be implemented by the student
        Storage_representation = c2p(v.T)
        return Storage_representation
    ...
    def o2s(self, v):
        ...
        """
        # TODO: To be implemented by the student
        Storage_representation = p2c(v.T)
        return Storage_representation

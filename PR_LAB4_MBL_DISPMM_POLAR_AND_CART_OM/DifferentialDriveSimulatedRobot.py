...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[],*args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        self.encoder_reading_frequency = 10 # frequency of encoder readings
        self.Re= np.diag(np.array([22 ** 2, 22 ** 2]))  # covariance of simulated wheel encoder noise

        self.Polar2D_feature_reading_frequency = 100  # frequency of Polar2D feature readings
        ...
        self.xy_feature_reading_frequency = 100  # frequency of XY feature readings
        ...
        self.yaw_reading_frequency = 1/500   # frequency of Yasw readings
        self.v_yaw_std = np.deg2rad(5)  # std deviation of simulated heading noise

        # Sensors sampling rate control
        self.last_encoder_reading_time = 0
        self.last_compass_reading_time = 0
        self.last_xy_feature_reading_time = 0
        self.last_polar_feature_reading_time = 0
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        # Constuct the variables needed to implement the motion model equation (eq:fs)
        dt = self.dt
        K = np.diag(np.array([0.5, 0.5, 0.5]))  # gain matrix
        eta_s_k_1 = Pose3D(xsk_1[0:3])  # previous robot pose
        nu_s_k_1 = np.array(xsk_1[3:6]).reshape(3,1)  # previous robot velocity
        nu_d = usk  # desired robot velocity
        
        # Sampling the motion model noise
        w_s_k = np.random.multivariate_normal(np.zeros(3), self.Qsk).reshape(3, 1)  # motion model noise
        
        # Velocity with noise
        nu_s_k_1_w = nu_s_k_1 * dt + (0.5 * w_s_k)  * (dt **2)  # velocity with noise
        eta_s_k = Pose3D.oplus(eta_s_k_1, Pose3D(nu_s_k_1_w))  # new robot pose with noise
        nu_s_k = nu_s_k_1 + K @ (nu_d - nu_s_k_1) + w_s_k * dt  # new robot velocity wit noise
        self.xsk = np.vstack((eta_s_k.reshape(3,1), nu_s_k.reshape(3,1)))  # new robot state
        

        if self.k % self.visualizationInterval == 0:
                print("Step:", self.k, "True pose:", self.xsk[0:3, 0], "Input:", usk.T)
                self.PlotRobot()
    ...
    def ReadPolarFeature(self, xk_bar):
        # Robot position
        robot_pose = Pose3D(self.xsk[0:3,0].reshape(3,1)) # robot pose
        zf = []  # List to store observed features
        Rf = []  # List to store covariances
        # reading interval
        delta_t = 1.0 / self.Polar2D_feature_reading_frequency

        # Reading the current time
        current_time = self.k * self.dt

        # if the time since last reading is less than the reading interval, return the previous reading
        if (current_time - self.last_polar_feature_reading_time) < delta_t:
            return zf, self.Rfp

        for feature in self.M:
            # Extract feature coordinates (assuming CartesianFeature is 2D)
            if isinstance(feature, f.CartesianFeature):
                z_local = c2p(feature.boxplus(robot_pose.ominus()))  # feature position in robot frame
            else:
                z_local = feature.boxplus(robot_pose.ominus())  # feature position in robot frame

            # Compute Euclidean distance
            distance = np.linalg.norm(z_local)

            if distance <= self.Polar2D_max_range:
                # Simulate measurement noise
                cov = self.Rfp  # Replace with your actual covariance if needed
                noise = np.random.multivariate_normal([0, 0], cov).reshape(2, 1)  # noise for range and bearing
                noisy_obs = z_local + noise
                
                zf.append(noisy_obs)
                Rf.append(cov)
        
        # Stack results for convenient downstream use
        if zf:
            zf = np.vstack(zf)
            Rf = scipy.linalg.block_diag(*Rf)  # <-- Use block_diag here, not np.stack

        self.last_polar_feature_reading_time = current_time

        return zf, Rf

    def ReadCartesianFeature(self, xk_bar):
        # Robot position
        # x_robot = self.xsk[0]
        # y_robot = self.xsk[1]
        # print("This is shape of self.xsk[0:3]", self.xsk[0:3,0].reshape(3,1).shape)

        robot_pose = Pose3D(self.xsk[0:3,0].reshape(3,1)) # robot pose
        zf = []  # List to store observed features
        Rf = []  # List to store covariances
        # print(f"This is type of zf {type(zf)}")
        # reading interval
        delta_t = 1.0 / self.xy_feature_reading_frequency
        ...
        for feature in self.M:
            ...
            if distance <= self.xy_max_range:
                # Simulate measurement noise
                cov = self.Rxy  # Replace with your actual covariance if needed
                # --- FIX 2: Reshape noise to match z_local (2,1) ---
                noise = np.random.multivariate_normal([0, 0], cov).reshape(2, 1)
                noisy_obs = z_local + noise
                
                zf.append(noisy_obs)
                # print("This is the shape of cov", cov.shape)
                Rf.append(cov)
        
        # Stack results for convenient downstream use
        # TODO: Check whether to stack zf horizontally or vertically based on expected output format
        if zf:
            # print("This is the number of observed features:", len(zf))
            zf = np.vstack(zf)
            Rf = scipy.linalg.block_diag(*Rf)  # <-- Use block_diag here, not np.stack
            # print("This is the shape of Rf after stacking", Rf.shape)
        # else:
        #     zf = None
        #     Rf = None
        self.last_xy_feature_reading_time = current_time
    ...
    def ReadEncoders(self):
        ...
        **To be completed by the student**.
        ...
        # reading interval
        delta_t = 1.0 / self.encoder_reading_frequency

        # Reading the current time
        current_time = self.k * self.dt

        # if the time since last reading is less than the reading interval, return the previous reading
        if (current_time - self.last_encoder_reading_time) < delta_t:
            return None, self.Re

        # Sampling the encoder noise
        noise_L, noise_R = np.random.multivariate_normal(np.zeros(2), self.Re).reshape(2, 1)  # left and right wheel encoder noise
        
        # Mapping robot linear and angular velocity to left and right wheel linear velocities
        v_r = self.xsk[3] + (self.wheelBase / 2) * self.xsk[5] # right wheel linear velocity
        v_l = self.xsk[3] - (self.wheelBase / 2) * self.xsk[5]  # left wheel linear velocity

        # Compute the wheel encoder readings (in pulses) given the wheel linear velocities
        n_R = (((v_r * delta_t) / (2 * np.pi * self.wheelRadius)) * self.pulse_x_wheelTurns)  + noise_R # right wheel encoder reading (in pulses)
        n_L = (((v_l * delta_t) / (2 * np.pi * self.wheelRadius)) * self.pulse_x_wheelTurns)  + noise_L # left wheel encoder reading (in

        zsk = np.array([[n_L], [n_R]])  # encoder readings
        Rsk = self.Re  # covariance of the encoder readings

        # update the last reading time
        self.last_encoder_reading_time = current_time
    ...
    def ReadCompass(self):
        ...
        # reading interval
        delta_t = 1.0 / self.yaw_reading_frequency
        ...
        # if the time since last reading is less than the reading interval, return the previous reading
        if (current_time - self.last_compass_reading_time) < delta_t:
            return None, self.v_yaw_std ** 2

        # Sampling the yaw noise
        noise_yaw = np.random.normal(0, self.v_yaw_std)  # yaw noise

        yaw = self.xsk[2] + noise_yaw # yaw with noise
        R_yaw = self.v_yaw_std ** 2  # covariance of the yaw noise

        # update the last reading time
        self.last_compass_reading_time = current_time
        return yaw, R_yaw

...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        # TODO: Slight confusion whether it would be self.hm or the compass observation with noise?
        h_list = []
        
        # --- FIX START ---
        # Only calculate expected measurement hm if zm was observed in this step
        # We use getattr with default True to be safe on first run or initialization
        if getattr(self, 'zm_observed', True): 
            print("Observed compass")
            h_list.append(self.hm(xk))

        if getattr(self, 'zf_observed', True): 
            print("Observed features")
            h_list.append(self.hf(xk))
        # hf is already dynamic because it iterates over self.H (which is updated in Localize)
        # h_list.append(self.hf(xk))
        
        # Stack whatever is in the list
        if len(h_list) > 0:
            h_mf = np.vstack(h_list)
        else:
            h_mf = np.array([]).reshape(0,1)

        # print(f"This is the shape of h_mf inside h function in FEKFMBL {h_mf.shape}")
        return h_mf
    ...
    def SquaredMahalanobisDistance(self, hfj, Pfj, zfi, Rfi):
        ...
        # TODO: To be completed by the student
        vij = zfi - hfj
        # print("This is the value of vij:", vij)
        Sij = Pfj + Rfi
        # print("This is the value of Sij:", Sij)
        # print("This is shape of Sij", Sij.shape)
        # print("This is shape of vij", vij.shape)
        D2_ij = vij.T @ np.linalg.inv(Sij) @ vij
        # print("This is the value of D2_ij:", D2_ij)
        return D2_ij
    ...
    def IndividualCompatibility(self, D2_ij, dof, alpha):
        ...
        """
        # TODO: To be completed by the student
        # TODO: Should it alpha or alpha-1
        threshold = scipy.stats.chi2.ppf(alpha, dof)
        # print("This is the value of threshold:", threshold)
        #TODO: if we have to take threshold square or not?
        isCompatible = D2_ij <= threshold
        # print("This is the value of isCompatible:", isCompatible)
        return isCompatible
    ...
    def ICNN(self, hf, Phf, zf, Rf, dim):
        ...
        H = [None] * (self.n_zf)
        # print(len(H))
        for i in range(len(zf) // dim):
            # print("Inisde the first loop in the ICNN function")
            min_D2 = float('inf')
            best_j = None
            # TODO Test whether the following slicing is needed or not, like if zfi is 
            # just a 1D vector flattened out
            zfi = zf[i*dim:(i+1)*dim]
            Rfi = Rf[i*dim:(i+1)*dim, i*dim:(i+1)*dim]
            # print("This is the shape of Rfi", Rfi.shape)
            # print("This is the shape of zfi", zfi.shape)
            # print(f"The shape of hf is {hf.shape}")
            for j in range(len(hf) // dim): # TODO should it be len(hf) or total number of features in the map?
                # print("Coming inside the second loop in the ICNN function")
                hfj = hf[j*dim:(j+1)*dim]
                # print("This is the shape of hfj", hfj.shape)
                Pfj = Phf[j*dim:(j+1)*dim, j*dim:(j+1)*dim]
                # print("This is the shape of Pfj", Pfj.shape)
                # print("Calling SquaredMahalanobisDistance inside the ICNN")
                D2_ij = self.SquaredMahalanobisDistance(hfj, Pfj, zfi, Rfi)
                if D2_ij < min_D2 and self.IndividualCompatibility(D2_ij, dim, self.alpha):
                    min_D2 = D2_ij
                    best_j = j
            # dof = dim
            # if self.IndividualCompatibility(min_D2, dof, self.alpha):
    ...
    def DataAssociation(self, xk, Pk, zf, Rf):
        ...
        # TODO: To be completed by the student
        # Build expected observations for ALL map features
        hf_list = []
        Phf_list = []


        for j in range(self.nf):  # self.nf = number of features in the map
            # Compute expected observation for map feature j
            hfj = self.hfj(xk, j)
            # print("This is the shape of hfj", hfj.shape)
            # Compute covariance of expected observation for map feature j
            Jhfj = self.Jhfjx(xk, j)
            # print("This is the shape of Jhfj", Jhfj.shape)
            Phfj = Jhfj @ Pk @ Jhfj.T
            # print("This is the shape of Phfj", Phfj.shape)

            hf_list.append(hfj)
            Phf_list.append(Phfj)

        # Stack all expected observations and covariances
        # print("This is hf_list length:", len(hf_list))
        # print("This is Phf_list length:", len(Phf_list))
        hf = np.vstack(hf_list) if hf_list else np.array([]).reshape(0, self.zfi_dim)
        # hf = self.hf(xk)
        # print("This is the shape of hf", hf.shape)
        Phf = scipy.linalg.block_diag(*Phf_list) if Phf_list else np.array([]).reshape(0, 0)
        # print("This is the shape of Phf", Phf.shape)
        # Now perform data association matching observed features with expected features
        # print("Calling ICNN inside the DataAssociation function")
        H = self.ICNN(hf, Phf, zf, Rf, self.zfi_dim)
        return H
    ...
    def Localize(self, xk_1, Pk_1):
        ...
        # print("This is the type and shape of zm, Rm, Hm, Vm:", type(zm), zm.shape if zm is not None else None,
        #       type(Rm), Rm.shape if Rm is not None else None,
        #       type(Hm), Hm.shape if Hm is not None else None,
        #       type(Vm), Vm.shape if Vm is not None else None)
        # Store a flag indicating if measurements (zm) are available for this step
        self.zm_observed = (zm is not None)
        

        zf, Rf = self.GetFeatures(xk_bar)
        # print(f"This is type of zf inside localize function in FEKFMBL {type(zf)}")
        if zf is None:
        ...
        # self.zf_observed = (zf is not None or len(zf)==0)
        # print("This is the length of zf:", len(zf)//self.zfi_dim if zf is not None else 0)
        
        # print("This is the type and shape of zf, Rf:", type(zf), zf.shape if zf is not None else None,
        #       type(Rf), Rf.shape if Rf is not None else None)
        self.n_zf = 0 if zf is None else len(zf) // self.zfi_dim
        # print("This is the value of self.n_zf:", self.n_zf)
        # print(f"This is zf {zf}")
        # print(f"This is shape of zf {zf.shape}")
        # print(f"This is shape of Rf {Rf.shape}")
        self.H = self.DataAssociation(xk_bar, Pk_bar, zf, Rf)
        # print("This is the type and shape of self.H:", type(self.H), self.H.shape if self.H is not None else None)
    ...
    def StackMeasurementsAndFeatures(self, zm, Rm, Hm, Vm, zf, Rf, H):
        ...
        # TODO: To be completed by the student
        # Split features into paired and non-paired sets
        if Hm is not None:
            # Determining the current full state dimension
            # We use self.xk_bar which should be set by Prediction()
            # If not available, we fall back to Hm's own width (no padding)
            full_state_dim = Hm.shape[1] 
            if hasattr(self, 'xk_bar') and self.xk_bar is not None:
                full_state_dim = self.xk_bar.shape[0]
            
            # If Hm corresponds only to the robot (e.g. width 3) but state is larger (e.g. 11)
            # We must pad Hm with zeros to the right.
            if Hm.shape[1] < full_state_dim:
                padding_width = full_state_dim - Hm.shape[1]
                zero_padding = np.zeros((Hm.shape[0], padding_width))
                Hm = np.hstack((Hm, zero_padding))
        zf_p, Rf_p, Hp, Vp, znp, Rnp = self.SplitFeatures(zf, Rf, H)
        

        if zf_p is None: # If no features matched
            zk, Rk, Hk, Vk = zm, Rm, Hm, Vm
        elif zm is None: # If no compass/encoder measurements
            zk, Rk, Hk, Vk = zf_p, Rf_p, Hp, Vp
        else:
            zk = np.block([[zm], [zf_p]])
            Rk = scipy.linalg.block_diag(Rm, Rf_p)
            Hk = np.block([[Hm], [Hp]])
            Vk = scipy.linalg.block_diag(Vm, Vp)
        # # Build lists of all components to be vertically (z_k, H_k) or block-diagonally (R_k, V_k) stacked
        # zk_list = []
        # Rk_components = []
        # Hk_components = []
        # Vk_components = []

        # # Case 1: Measurements (zm) are present (assumed to be independent of feature observations)
        # if zm is not None:
        #     zk_list.append(zm)
        #     Rk_components.append(Rm)
        #     Hk_components.append(Hm)
        #     Vk_components.append(Vm)

        # # Case 2: Paired features (zf_p) are present
        # if zf_p is not None:
        #     zk_list.append(zf_p)
        #     # Rf_p is already a single block-diagonal matrix for all paired features
        #     Rk_components.append(Rf_p) 
        #     # Hp is now a single stacked matrix, use append()
        #     Hk_components.append(Hp)   
        #     # Vp is now a single block-diagonal matrix, use append()
        #     Vk_components.append(Vp)   
        ...
        #     zk = Rk = Hk = Vk = None
        # else:
        #     # Stack observation vectors (zk)
        #     zk = np.vstack(zk_list)
            
        #     # Stack covariance matrices Rk and Vk (block-diagonal, assuming independence)
        #     # Rk_components contains Rm and Rf_p, which are both single matrices.
        #     Rk = scipy.linalg.block_diag(*Rk_components)
            
        #     # Stack observation matrices Hk (vertical stacking)
        #     # Hk_components contains Hm and Hp, which are both single matrices.
        #     Hk = np.vstack(Hk_components)
            
        #     # Stack noise observation matrices Vk (block-diagonal)
        #     # Vk_components contains Vm and Vp, which are both single matrices.
        #     Vk = scipy.linalg.block_diag(*Vk_components)
    ...
    def SplitFeatures(self, zf, Rf, H):
        ...
        for i in range(len(H)):
            zfi = zf[i*self.zfi_dim:(i+1)*self.zfi_dim]
            Rfi = Rf[i*self.zfi_dim:(i+1)*self.zfi_dim, i*self.zfi_dim:(i+1)*self.zfi_dim]
            if H[i] is not None:
                zp_list.append(zfi)
                Rp_list.append(Rfi)

                # TODO: Whether to pass xk or xk_bar?
                # print(f"This is the shape of self.Jhjfx(self.xk_bar, H[i]) {self.Jhfjx(self.xk_bar, H[i]).shape}")
                Hp.append(self.Jhfjx(self.xk_bar, H[i]))
                Vp.append(np.eye(self.zfi_dim))  # Assuming identity for the noise observation matrix of features
            else:
                znp_list.append(zfi)
                Rnp_list.append(Rfi)

        # TODO: Check if the following concatenations are correct
        zp = np.vstack(zp_list) if zp_list else None
        # print("This is the shape of zp:", zp.shape if zp is not None else None)
        # Rp = np.block([[Rp_list[i] if i == j else np.zeros_like(Rp_list[0]) for j in range(len(Rp_list))] for i in range(len(Rp_list))]) if Rp_list else None
        Rp = scipy.linalg.block_diag(*Rp_list) if Rp_list else None
        # print("This is the shape of Rp:", Rp.shape if Rp is not None else None)

        # 3. Paired Jacobian Matrix (Hp): Vertical stacking
        Hp = np.vstack(Hp) if Hp else None
        
        # 4. Paired Noise Matrix (Vp): Block-diagonal stacking
        Vp = scipy.linalg.block_diag(*Vp) if Vp else None
        znp = np.vstack(znp_list) if znp_list else None
        # print("This is the shape of znp:", znp.shape if znp is not None else None)
        # Rnp = np.block([[Rnp_list[i] if i == j else np.zeros_like(Rnp_list[0]) for j in range(len(Rnp_list))] for i in range(len(Rnp_list))]) if Rnp_list else None
        Rnp = scipy.linalg.block_diag(*Rnp_list) if Rnp_list else None
        # print("This is the shape of Rnp:", Rnp.shape if Rnp is not None else None)
    ...
    def PlotFeatureObservationUncertainty(self, zf, Rf, color):  # plots the feature observation uncertainty ellipse
        ...
        :param Rf: covariance matrix of the feature observations
        """
        print("Plotting Feature Observation Uncertainity")
        ...
        for i in range(0, nzf):
            ...
            NxF = self.g(NxB, BxF)
            # print(f"This is the value NxF {NxF}")
            J = self.Jgv(NxB, BxF)
            NRf = J @ BRF @ J.T
    ...
    def PlotExpectedFeaturesObservationsUncertainty(self):
        ...
        for Fj in range(self.nf):   # for all map features
            ...
            robot_pose = self.GetRobotPose(xk=self.xk)

            Nhx_Fj = self.g(robot_pose, h_Fj) # expected feature observation in the N-Frame in storage space
            #Jx = self.Jgx(self.xk, h_Fj)
            Jv = self.Jgv(robot_pose, h_Fj)
